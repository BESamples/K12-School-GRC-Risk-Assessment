...
$logPath = Join-Path $PSScriptRoot "Northstar-AD-Setup-$timestamp.log"
$reportPath = Join-Path $PSScriptRoot "Northstar-AD-Build-$timestamp.csv"

Start-Transcript -Path $logPath -Force | Out-Null

try {
    Write-Host "Building Northstar lab objects in domain: $($script:Domain.DNSRoot)" -ForegroundColor Yellow
    ...
    Write-Step 'Creating the service and disabled accounts'

    Ensure-User -GivenName 'Wazuh' -Surname 'Service' -DisplayName 'Wazuh Service' -SamAccountName 'svc_wazuh' -Path $ouServiceAccounts -Description 'Simulated Wazuh service account' -PasswordNeverExpires $true -CannotChangePassword $true -ChangePasswordAtLogon $false
    ...
    Write-Host "Service-account risk test: svc_wazuh has a non-expiring password." -ForegroundColor Yellow
}
catch {
    Write-Host "`nNorthstar lab setup failed." -ForegroundColor Red
